...
import com.group8.gui.ClientGUI;
import com.group8.logger.ProgLogger;
import com.group8.server.ChatroomServer;

/**
 * Client class that handles all logic for a user in this multi-chatroom application. A client
 * connects to a LookUpServer which holds information about existing chat rooms that the client can
 * connect to. A client can sign in/sign up an account, join/create a chatroom, and send messages and
...
public class Client {
    ...
    /**
     * Attempt to log out while on the chat selection screen. This involves sending a message to the
     * LookUp server to log out the user. If the socket fails then we attempt to contact another
    ...
    /**
     * Attempt to log out while in a chatroom. This involves sending a message to the LookUp server to
     * log out the user. If the client is a host then the LookUp server will coordinate choosing a new
     ...
     * the chatroom. These receivers are usually used for receiving chatroom messages, but it is also used to
    ...
    private class MulticastMessageReceiver implements Runnable {
        ...
        public void run() {
            try {
                ...
                multicastSocket.joinGroup(new InetSocketAddress(groupIP, 0), null);
        ...
        public void turnOff() {
            isAlive = false;
            try {
                multicastSocket.leaveGroup(new InetSocketAddress(groupIP, 0), null);
            } catch (IOException e) {
     ...
     *
     * @return an arraylist of the strings pairings of the chat name and number of users
     */
