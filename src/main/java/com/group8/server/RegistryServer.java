...
/**
 * Registry server class that orchestrates connecting all the LookUp servers by sockets. The
 * registry server listens for LookUp servers to connect to it, and then tells all the already
...
public class RegistryServer {
    ...
    public List<PaxosServerInfo> paxosServerInfos;
    public int myPort;
    public String myAddress;

    /**
     * Constructor for registry server that creates an arraylist of LookUp server info (paxos servers),
    ...
    public RegistryServer() {
        try {
            this.paxosServerInfos = Collections.synchronizedList(new ArrayList<PaxosServerInfo>());
    ...
    private class NewServerSocketHandler implements Runnable {
        ...
        public void run() {
            try {
                BufferedReader newServerReader = new BufferedReader(
                        new InputStreamReader(this.newServerSocket.getInputStream()));
                BufferedWriter newServerWriter = new BufferedWriter(
                        new OutputStreamWriter(this.newServerSocket.getOutputStream()));
                String line = newServerReader.readLine();
                String[] messageArray = line.split("@#@");
                String newServerAddress = messageArray[0];
                int newServerPort = Integer.parseInt(messageArray[1]);
                String newServerPaxosRole = messageArray[2];
                PaxosServerInfo newPaxosServerInfo = new PaxosServerInfo(newServerAddress,
                        newServerPort, this.newServerSocket, newServerWriter);
                // Tell each server that has already connected to make a socket connection to the new server
                synchronized (paxosServerInfos) {
                    Iterator<PaxosServerInfo> i = paxosServerInfos.iterator();
                    while (i.hasNext()) {
                        PaxosServerInfo info = (PaxosServerInfo) i.next();
                        info.writer.write("startConnection@#@" + newServerAddress + "@#@"
                                + newServerPort + "@#@" + newServerPaxosRole);
                        info.writer.newLine();
                        info.writer.flush();
                    }
                }
