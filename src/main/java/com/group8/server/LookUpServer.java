...
public class LookUpServer {
    ...
    public void doLogoutTransaction(String[] transactionInfo) {
        ...
        for (Map.Entry<String, ChatroomInfo> chatNameChatroomInfo : chatNameChatroomInfoStore.entrySet()) {
    ...
    /**
     * Carry out a creation chat transaction by creating a new chatroomInfo objet and setting the ID,
     * host username, chat name, groupIP, address for new members to connect to chatroom server, and
     * adding the host user as a member. Then add the chatroomInfo object to collection of chatroomInfo.
     *
     * @param transactionInfo
     */
    ...
    /**
     * Carry out update chat connection port transaction by setting the port of the chatroomInfo object
    ...
    private class PaxosSocketMessageReceiver implements Runnable {
        ...
        /**
         * Handle a prepared message from another LookUp server by checking the proposal number and not
        ...
        public void handlePromise(String[] messageArray) {
            ...
            if (acceptorLookUpServersReadersWriters.size() / 2 <= numPromisedAcceptors && !reachedMajorityPromises) {
                ...
                for (Map.Entry<BufferedReader, BufferedWriter> acceptor : acceptorLookUpServersReadersWriters.entrySet()) {
         ...
         * accepted. If a majority has then notified all the learners (proposers and acceptors are also
         * learners) pf what transaction to carry out.
         *
         * @param messageArray
         */
        public void handleAcceptResponse(String[] messageArray) {
            Long.parseLong(messageArray[1]);
            ...
            if (acceptorLookUpServersReadersWriters.size() / 2 <= numAcceptedAcceptors && !reachedMajorityAcceptances) {
                ...
                // modify all acceptors of what transaction they should carry out,
                for (Map.Entry<BufferedReader, BufferedWriter> acceptor : acceptorLookUpServersReadersWriters.entrySet()) {
                    BufferedWriter acceptorWriter = (BufferedWriter) acceptor.getValue();
                    try {
                ...
                for (Map.Entry<BufferedReader, BufferedWriter> proposer : proposerLookUpServersReadersWriters.entrySet()) {
                ...
                for (Map.Entry<BufferedReader, BufferedWriter> learner : learnerLookUpServersReadersWriters.entrySet()) {
    ...
    private class RegistryServerListener implements Runnable {
        private final Socket registrySocket;
        ...
        public void run() {
            try {
                ...
                BufferedWriter registryWriter = new BufferedWriter(
                ...
                registryWriter.write(addressForOtherPaxosServersToConnectTo + "@#@" + portForOtherPaxosServersToConnectTo + "@#@" + myPaxosRole);
    ...
    private class InstigateSocketConnectionToOtherPaxosServer implements Runnable {
        ...
        private final Socket otherPaxosServerSocket;
    ...
    private class ClientSocketHandler implements Runnable {
         ...
         * and send out prepare requests to all the acceptor LookUp servers. The acceptors' responses
         ...
         * for the chatroomInfo, and setting up a socket for the chatroom server to connect to receive
         ...
         * name. If the chat name already exists then notify client that it exists. Else, start a paxos
                ...
                ActionListener listener = event -> {
                    try {
                        ...
                        if (newHost == null) {
                            ...
                            // message will recreate chatroom with the given chat name.
                            try {
        ...
        private class ChatroomListener implements Runnable {
            ...
            private final BufferedWriter chatroomWriter;
            private final BufferedReader chatroomReader;
            ...
            /**
             * When host has left chatroom by clicking logout, stop the heartbeat timer for the chatroom
             ...
             * @throws IOException
             */
            public void handleChatroomServerHostChatroomLogout() throws IOException {
             ...
             * @throws IOException
             */
            public void handleChatroomServerHostBackToChatSelection() throws IOException {
                ...
                while (isAlive) {
                    ...
                    try {
                        ...
                        if (messageArray[0].equalsIgnoreCase("messageSent")) {
                            ...
                            handleChatroomServerHostChatroomLogout();
                            ...
                            handleChatroomServerHostBackToChatSelection();
                        }
                    } catch (IOException e) {
                        isAlive = false;
        ...
        private String handleNotifyMembersOfRecreation(String[] notifyMessage) {
            ...
            // send multicast message to all remaining members of chatroom of whom new host is and what
         ...
         *
         * @param usersRequest
         * @return a string either indicating the chatroom does not exist or indicating all the
        ...
        private String handleGetNumUsersInChatrooms() {
            ...
            for (Map.Entry<String, ChatroomInfo> nameInfoPair : chatNameChatroomInfoStore.entrySet()) {
            ...
            logger.logger.info("Handled request to get number of users in all chat rooms");
    ...
    public static void main(String[] args) {
        ...
        new LookUpServer(lookUpPort0, 0, registryAddress, registryPort, "proposer");
        new LookUpServer(lookUpPort1, 1, registryAddress, registryPort, "proposer");
        new LookUpServer(0, 2, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 3, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 4, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 5, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 6, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 7, registryAddress, registryPort, "acceptor");
        new LookUpServer(0, 8, registryAddress, registryPort, "learner");
        new LookUpServer(0, 9, registryAddress, registryPort, "learner");
        new LookUpServer(0, 10, registryAddress, registryPort, "learner");
    }
}
